...
type AdminRoleRepository interface {
	repository.Repository[model.AdminRole] // 继承通用方法
...
type AdminRoleRepositoryImpl struct {
	*repository.BaseRepository[model.AdminRole] // 内嵌字段，继承方法提升
}

func NewAdminRoleRepository(db *gorm.DB) AdminRoleRepository {
	return &AdminRoleRepositoryImpl{
		BaseRepository: repository.NewBaseRepository[model.AdminRole](db),
	}
}
