...
func NewStaffService(db *gorm.DB, staffRepository repository.StaffRepository) StaffServiceInterface {
	return &StaffService{
		db:              db,
...
// Page 分页获取仓库人员列表
func (s *StaffService) Page(pageRequest base_request.PageRequest) ([]model.Staff, int64, error) {
	// 创建分页参数
	...
	// 查询数据
	count, staffs, err := s.staffRepository.Page(base_repository.ConditionScope{}, pagination)
	if err != nil {
		return nil, 0, err
	}
...
// Create 创建仓库人员
func (s *StaffService) Create(model *model.Staff) (*model.Staff, error) {
	// 检查姓名是否已存在
	...
	find, err := s.staffRepository.FindBy(cond)
	if err == nil && find != nil {
		return nil, errors.New("该姓名的仓库人员已存在")
	}

	err = s.staffRepository.Create(model, nil)
	if err != nil {
		return nil, err
	}
...
// Update 更新仓库人员信息
func (s *StaffService) Update(model *model.Staff) (*model.Staff, error) {
	// 保存更新
	err := s.staffRepository.Update(model, nil)
	if err != nil {
		return nil, err
	}
...
func (s *StaffService) Delete(id uint) error {
	return s.staffRepository.DeleteById(id, nil)
}

// GetStaff 根据 ID 获取仓库人员
func (s *StaffService) GetStaff(id uint) (*model.Staff, error) {
	return s.staffRepository.FindById(id)
}

// UpdateStaff 更新仓库人员信息
func (s *StaffService) UpdateStaff(id uint, request request.StaffUpdateRequest) (*model.Staff, error) {
	// 获取现有仓库人员
	...
	if request.Name != "" && request.Name != staff.Name {
		existingStaff, err := s.staffRepository.FindByName(request.Name)
	...
	// 保存更新
	err = s.staffRepository.Update(staff, nil)
	if err != nil {
		return nil, err
	}
...
// ListStaffs 获取仓库人员列表
func (s *StaffService) ListStaffs(filter request.StaffFilterRequest) ([]model.Staff, int64, error) {
	// 创建查询条件
	cond := base_repository.ConditionScope{
	...
	// 查询数据
	count, staffs, err := s.staffRepository.Page(cond, pagination)
	if err != nil {
		return nil, 0, err
	}
...
func (s *StaffService) UpdateStaffState(id uint, state model.StaffState) (*model.Staff, error) {
	// 验证状态值
	if !state.IsValid() {
